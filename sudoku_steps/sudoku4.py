...
class Sudoku:
    ...
    def __init__(self, puzzle: Iterable[Iterable]):
        ...
        self._empty_values: list[tuple[int, int]] = []
    ...
    def load_row_columns(self) -> None:
        for y in range(9):
            for x in range(9):
                ...
                if value == 0 and (x, y) not in set(self._empty_values):
                    self._empty_values.append((x, y))
    ...
    def unplace(self, x: int, y: int) -> None:
        ...
        self.place(0, x, y)
        if (x, y) not in set(self._empty_values):
            self._empty_values.append((x, y))
